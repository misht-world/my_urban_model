...
class WC(str, Enum):
    ...
    # Плотность населения превышает норматив (300 чел/га по СП 42.13330.2026).
    ...
    # Доп. образование в ВПП при местах ≥ порога → дробление на несколько
    ...
    # v0.19.1: пользовательский объект по ВРИ совпадает с нормативным
    # соцобъектом, который модель считает сама (ДОО/СОШ/доп.обр/поликлиника/
    # спорт) → объект может быть учтён ДВАЖДЫ: и как норматив, и как
    # пользовательский. Информационное — модель не может знать намерение.
    CUSTOM_OBJECT_MAY_DUPLICATE = "CUSTOM_OBJECT_MAY_DUPLICATE"
