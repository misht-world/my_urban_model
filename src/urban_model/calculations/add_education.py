...
def compute(
    ...
    *,
    ...
    Args:
        ...
        norms: нормативы.
        mode: "norm" — места и размещение по нормативу; "manual" — ручное.
        places_override: число мест в ручном режиме.
        in_vpp_manual: разместить в ВПП (ручной режим).
    """
    ...
    threshold = norms.resolve("social_objects.add_education.built_in_threshold")
    ...
    if places <= 0:
        return AddEducationResult(0, 0, False, 0.0, 0.0, 0.0, 0)

    workers = max(1, round(places / workers_ratio))

    # Размещение: norm → авто по порогу; manual → по переключателю пользователя.
    if mode == "manual":
        built_in = bool(in_vpp_manual)
    else:
        built_in = places < threshold
