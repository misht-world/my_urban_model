...
def variant_rail(slide, labels: list[str], current: int | None) -> None:
    ...
    - На слайде сравнения (`current is None`) показываем ВСЕ ушки (обзор).
    - На детальном слайде варианта показываем ТОЛЬКО его ушко на своей позиции.
    Активный — тёмно-серый, впритык к правому краю; неактивные (в обзоре) —
    светло-серые. Только оттенки серого."""
    n = len(labels)
    if n == 0:
        return
    ...
    for i, label in enumerate(labels):
        active = (i == current)
        # Детальный слайд: рисуем только своё ушко; обзор (current None) — все.
