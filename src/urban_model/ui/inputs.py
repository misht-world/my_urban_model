...
def _render_essentials() -> tuple[Site, int, bool, "float | None", bool, bool, list]:
    ...
    if True:
        ...
        enforce_greening = st.checkbox(
            ":material/grass: Соблюдать озеленение ТОП ≥ 6 м²/чел",
            value=True, key="enforce_quarter_greening_norm",
            help=(
                "СП 42.13330.2026, прим. 4 табл. М.1: при ДПТ в жилых зонах — "
                "озеленённые территории общего пользования (ТОП) не менее "
                "6 м²/чел (в т.ч. 1,7 по СП 476). Заменил прежний контроль "
                "«25% площади квартала». ЗНОП засчитывается в эти 6 м²/чел; "
                "недостаток покрывается свободным резервом квартала. "
                "Выключите при компенсации озеленения вне границ территории."
            ),
        )
