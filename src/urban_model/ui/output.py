...
def render_header(result: TEPResult) -> None:
    ...
    if feasible_all:
        ...
        if bal.surplus >= 0 and bal.greening_actual < bal.greening_required - 1e-3:
            ...
            _spec_alert(
                "error", "error",
                f"**Норматив озеленения ТОП не выполняется.** "
                f"Требуется ≥ {fmt_int(bal.greening_required)} м² "
                f"(6 м²/чел общего пользования), факт "
                f"{fmt_int(bal.greening_actual)} м² (ЗНОП + резерв) — "
                f"дефицит {fmt_int(deficit)} м²."
            )
            _spec_alert(
                "info", "lightbulb",
                "Возможные действия: (1) включите/увеличьте **ЗНОП** в левой "
                "колонке — он засчитывается в норматив 6 м²/чел; "
                "(2) увеличьте площадь квартала; "
                "(3) отключите **«Соблюдать озеленение ТОП ≥ 6 м²/чел»** — "
                "если озеленение компенсируется вне границ территории."
            )
        else:
            _spec_alert(
                "error", "error",
    ...
    # v0.10.9 (#1): когда ограничивает норматив плотности 300 чел/га и при этом
