...
def _render_social_count_card(scan: ScanResult) -> None:
    """Карточка ДОО/СОШ: таблица «число объектов → вместимость/ЗУ/квартиры».

    Зависимость площади квартир от числа объектов задаётся ФОРМОЙ норматива
    ЗУ/место (v0.12.27):
      • СОШ — ставка падает с размером корпуса (piecewise) + бассейн/спорт-ядро
        на каждый корпус → меньше корпусов = меньше ЗУ = больше квартир;
      • ДОО — ставка почти плоская на всём диапазоне вместимостей → ЗУ ≈
        const × мест, квартиры от числа ДОО почти не зависят (это норматив,
        не упрощение модели).
    Таблица показывает и реализуемость (валидность вместимостей), и цену
    вопроса в м² квартир.
    """
    import re as _re
    ...
    if is_kg:
        st.caption(
            "У отдельно стоящих ДОО норматив ЗУ на место почти не меняется с "
            "размером здания, поэтому суммарный ЗУ ≈ const × мест — площадь "
            "квартир от числа ДОО практически не зависит. Таблица нужна для "
            "проверки допустимых вместимостей."
        )
    else:
        st.caption(
            "Норматив ЗУ на место у СОШ падает с размером корпуса, а бассейн и "
            "спорт-ядро добавляются на КАЖДЫЙ корпус — поэтому меньше корпусов "
            "= меньше ЗУ = больше площади квартир. Вместимости выбираются по "
            "минимуму ЗУ из типовых."
        )
...
def _render_scan_card(scan: ScanResult) -> None:
    ...
    if not scan.points:
        ...
        return
    # ДОО/СОШ — таблица (вместимости/ЗУ/квартиры/валидность), не график.
