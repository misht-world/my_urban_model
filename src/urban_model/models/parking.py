...
class ParkingBreakdown(BaseModel):
    ...
    # v0.12.2: стилобат — м/м и площадь деки. ЗУ квартала НЕ занимает
    # (стоит над пятном домов/двором), поэтому в total_surface_area не входит.
    stylobate_places: int = Field(default=0, description="М/м в стилобате")
    stylobate_area: float = Field(default=0.0, description="Площадь деки стилобата, м²")

    @property
    def total_surface_area(self) -> float:
        """Суммарная наземная площадь парковок (открытые + многоуровневые).
        Стилобат не входит — он не занимает ЗУ квартала."""
