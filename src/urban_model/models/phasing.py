...
Квартал делится на 2–8 очередей долями площади. Расчёт квартала не меняется:
...
class PhasingSpec(BaseModel):
    ...
    mode="manual" — пользователь задаёт доли площади (Σ = 1, 2–8 очередей).
    """
    ...
    def _validate_shares(cls, v: list[float]) -> list[float]:
        import math
        if not 2 <= len(v) <= 8:
            raise ValueError(f"очередей должно быть 2–8, задано {len(v)}")
