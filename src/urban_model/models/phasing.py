...
Квартал делится на 2–20 очередей долями площади. Расчёт квартала не меняется:
...
class PhasingSpec(BaseModel):
    ...
    mode="manual" — пользователь задаёт доли площади (Σ = 1, 2–20 очередей).
    """
    ...
    def _validate_shares(cls, v: list[float]) -> list[float]:
        import math
        if not 2 <= len(v) <= 20:
            raise ValueError(f"очередей должно быть 2–20, задано {len(v)}")
