...
class CalculationOptions(BaseModel):
    ...
    # На малых кварталах (< 0.5 га) норматив озеленения ТОП (6 м²/чел) и
    # норматив плотности 300 чел/га физически противоречивы, бисекция падает в
    # kit_search_min. Отключение даёт пользователю режим «физический
    # потолок» — модель максимизирует КИТ только в рамках территории.
    # NB: имя поля историческое (было про 25% квартала); с v0.20.2 контролирует
    # норматив озеленённых ТОП ≥ 6 м²/чел (СП 42.13330.2026, прим. 4 табл. М.1).
    enforce_quarter_greening_norm: bool = Field(
        default=True,
        description=(
            "Соблюдать озеленение ТОП ≥ 6 м²/чел (СП 42.13330.2026, "
            "прим. 4 табл. М.1). Отключите для малых кварталов или при "
            "компенсации озеленения вне границ территории."
        ),
    )
    enforce_density_norm: bool = Field(
        default=True,
        description=(
            "Соблюдать норматив плотности 300 чел/га (по 30 м²/чел, "
            "СП 42.13330.2026 п. 6.2.16). "
    ...
    # v0.18.0: схема расчёта внутриквартальных проездов.
    #   "by_objects"    — гибрид (по умолчанию): S_квартала × базовая доля
    #                     + подъезды к каждому объекту (соцобъекты 600,
    #                     иные отд. стоящие 300, инженерные 150, МУ-паркинги 120).
    #                     Откалибровано по фактическим проектам.
    #   "quarter_share" — только доля от квартала (схема до v0.18, для сверки
    #                     и старых проектов).
    # `driveways_intra_share_override` работает в ОБОИХ режимах: заменяет
    # базовую долю (в by_objects подъезды считаются сверх неё).
    driveways_intra_mode: Literal["by_objects", "quarter_share"] = Field(
        default="by_objects",
        description="Схема внутриквартальных проездов: гибрид по объектам или доля от квартала",
    )
    driveways_intra_share_override: float | None = Field(
        default=None,
        ge=0.0,
        le=0.5,
        description="Доля внутриквартальных проездов от S_квартала. None = норматив",
    )
